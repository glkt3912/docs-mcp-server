...
**RPC（Remote Procedure Call）** は、別プロセス・別マシン上の関数を、まるでローカルの関数を呼ぶように実行する仕組みです。

### ローカル呼び出しとの比較

```text
【ローカル呼び出し】
関数呼び出し → 即座に実行 → 戻り値を受け取る

【RPC】
関数呼び出し → シリアライズ → ネットワーク/IPC 送信
            → 相手側で実行 → 結果をシリアライズ → 返送 → デシリアライズ → 戻り値を受け取る
```

呼び出し側からは「関数を呼んだら結果が返ってきた」ように見えますが、
裏側では通信が発生しています。

### MCP における RPC

MCP では Claude Code（クライアント）が docs-mcp-server（サーバー）のツールを呼び出します。

```text
Claude Code                        docs-mcp-server
    │                                    │
    │  "search_metadata を実行して"      │
    │ ──────────────────────────────→   │
    │                                    │  loadMetadata()
    │                                    │  scoreDocument()
    │  "検索結果: ..."                   │
...
Claude Code 側は `search_metadata` がどこで・どう動くかを知らなくても呼び出せます。
これが RPC の本質です。
...
Unix 系 OS では、すべてのプロセスが起動時に3つの標準ストリームを持ちます。

| ストリーム | 略称 | 用途 |
| --------- | ---- | ---- |
| 標準入力 | `stdin` | プロセスへのデータ入力 |
| 標準出力 | `stdout` | プロセスからの通常出力 |
| 標準エラー出力 | `stderr` | エラーやログの出力 |

### stdin/stdout のパイプ接続

Claude Code は MCP サーバーを子プロセスとして起動し、**stdin/stdout をパイプで接続**します。

```text
Claude Code（親プロセス）          docs-mcp-server（子プロセス）
        │                                   │
        │  stdin  ──────────────────→       │  リクエスト受信
        │  stdout ←──────────────────       │  レスポンス送信
        │  stderr ←──────────────────       │  ログ（Claude Code が表示）
```

- `stdin` にリクエスト（JSON-RPC）を書き込む → サーバーが読み取って処理
- `stdout` にレスポンスを書き込む → Claude Code が読み取って解釈
- `stderr` はプロトコルと無関係 → デバッグログに使用

### なぜ stdout にログを書いてはいけないか

`stdout` はプロトコルメッセージ専用です。`console.log()` で余分な文字列を混入させると
JSON-RPC のパースが壊れ、通信が切断されます。

```typescript
// ✅ ログは stderr へ
console.error("[info] 処理開始");
